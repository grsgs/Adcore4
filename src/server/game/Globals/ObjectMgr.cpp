...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (min_value == MIN_TRINITY_STRING_ID)
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Depthscore strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR(LOG_FILTER_SQL, "Entry %i not found in `depthscore_string` table.", entry);
    else
